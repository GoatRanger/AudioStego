 ...
 * Window - Preferences - Java - Code Generation - Code and Comments
 */
public class FootballGame extends eecs.RobotGuiBase {
		
	static FootballTeam homeTeam, visitingTeam,offensiveTeam, defensiveTeam;
	...
	static {
		...
		driveSummaryMSG="";
		Football.playGame();
		homeTeam=Football.getHomeTeam();
		homeTeam.setName(Football.names[1]);
		visitingTeam=Football.getVisitingTeam();
		visitingTeam.setName(Football.names[0]);
	...
	public static void runTestModule()
	 {
		...
		int[] testDownData={1,2,3,4};
		int[] testYTGData={10,10,10,5};
		String[] testOffPlayResult={"RUN","PASS_SHORT","PASS_LONG","FG"};
		String[] testDefPlayResult={"ZONE","BLITZ","zone","BLITZ"};
		int numberOfTests=testDownData.length;
		int numberCorrect=0;
		for (int i=0;i<numberOfTests;i++)
		{
		    String currentOffPlayResult=defensiveTeam.offensiveStrategy(testDownData[i],testYTGData[i]);
		    String currentDefPlayResult=defensiveTeam.defensiveStrategy(testDownData[i],testYTGData[i]);// test the same team..
	...
	public static void setUpDisplay(){
		homeTeamMSG=new Message(Football.names[1]);
		sim.add(homeTeamMSG,0,15);
		visitingTeamMSG=new Message(Football.names[0]);
	...
	public void setHomeTeam(FootballTeam team)
	{
		homeTeam=team;
	}
	
	public void setVistingTeam(FootballTeam team)
	{
		visitingTeam=team;
	}
	...
	public static void newDrive()
	{
		...
		if (robot!=null)
			destroyRobot();
		createRobot();
		//Image bg = ImageLoader.getImage(Football.class, "/eecs/instructor/cobb/helmet.gif");
		...
		while (!endOfDrive)
		{
			...
			String offPlay=offensiveTeam.offensiveStrategy(currentDown, yardsToGo);
			String defPlay=defensiveTeam.defensiveStrategy(currentDown, yardsToGo);
