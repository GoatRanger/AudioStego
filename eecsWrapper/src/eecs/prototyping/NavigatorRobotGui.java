 ...
 * @version 0.1
 */
public class NavigatorRobotGui extends eecs.RobotGuiBase
{
    ...
    static
    {
        sim = new eecs.simulation.DemoPolySim();
        setSimulationTitle("IT105 Final Project Test Area");
    ...
    {
        printLine("In the prototype, you cannot change the polygon.");
        ((DemoPolySim) sim).createPolygon(false, false);
    }

    /**
     * Creates a polygon in the simulation that is convex.  Only one
     * vertex will be concave, and it will be marked with a red
     * vertex.
     */
    ...
    /**
     * Creates a polygon that may be either concave or convex.  The
     * constraints are the same as specified in {@link
     * #setConcavePolygon()} and {@link setConvexPolygon}
     */
    ...
    /**
     * Returns the Y values of the vertices of the polygon, as an
     * array of  integers.
     *
     * @return DOCUMENTATION INCOMPLETE
     */
     ...
     * rotation and forward/backward movement.  Upon completion,
     * this will move the robot to originX, originY, with a heading
     * of 25.
     */
    public static void calibrate()
    {
        ...
        double scale =
            ((TimingNavigatorRobot) robot).getMovementScale();
        ...
        double error =
            (originX - loc.getX()) + (originY - loc.getY());
        ...
        while (error > 3)
        {
            ...
            error =
                Math.abs(originX - loc.getX()) +
                Math.abs(loc.getY() - originY);
