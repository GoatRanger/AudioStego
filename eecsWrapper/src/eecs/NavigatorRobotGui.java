...
public class NavigatorRobotGui extends eecs.RobotGuiBase
{
    ...
    public static void transmitResult(int type, int x, int y)
    {
        ...
        if (((xdiff < 6) && (ydiff < 6) && (type == CONCAVE)) ||
              ((index > 10) && (type == CONVEX)))
        {
        ...
        else
        {
            ...
            {
                printLine(
                    "Polygon is Convex, you transmitted concave");
            }
            else
            {
                printLine(
                    "Appears to be incorrect (outside tolerance):");
                printLine("Your point: (" + x + ", " + y + ")");
                printLine(" Actual values: (" + getXPoint(index) +
                    ", " + getYPoint(index) + ")");
            }
        }
    }
