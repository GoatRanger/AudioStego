 ...
 * 
 * @author Slava Pestov
 * @version $Id: JEditTextArea.java,v 1.5 2004/06/10 17:31:12 dk8685 Exp $
 */
public class JEditTextArea extends JComponent {
  ...
  public void updateScrollBars() {
    if (vertical != null && visibleLines != 0) {
      ...
      // Assuming the number of lines in the document changed, update
      // the line numbering area, and relayout the container to update
      // to a potentially changed preferred width of the line numbering area..
      lineNumbering.setPreferredSize(getLineCount());
  ...
  /* TODO: This is broken. _offsetToX isn't returning the right value for widths,
  *       so the position of the caret is off.  Something with the FontMetrics/Font
  *       that it's getting the wrong info.  Can disable for now, since the font is
  *       a monospaced font, so every character IS the exact same width.
  */
    ...
    try {
      if (rectSelect) {
        ...
        if (selectedText.indexOf("\t")>=0) {
            int numSpaces = painter.tabSize/painter.getFontMetrics().charWidth(' ');
            StringBuffer spaces = new StringBuffer();
            for (int i=0; i<numSpaces; i++) {
             spaces.append(" ");   
            }
            selectedText = selectedText.replaceAll("\\\t",spaces.toString());
        }        
...
//            .getTransferData(DataFlavor.stringFlavor)).replace('\r', ' ');
   ...
   *          A boolean that is <code>true</code> if the search should be case
   *          sensitive.
   * @param wholeWordOnly
   *          A boolean that is <code>true</code> if the search should only
   *          match whole words.
   ...
   *          a boolean indicating if the search should only replace when the
   *          searchPattern matches a complete word (not a substring)
   ...
   * @param matchWholeWords
   *          <code>true</code> only if the pattern should only match whole
   *          words
  ...
  class ScrollLayout implements LayoutManager {
    ...
    public void layoutContainer(Container parent) {
      ...
      right.setBounds(ileft + gutterWidth + centerWidth, itop, rightWidth,
          centerHeight);
  ...
  /**
   * Line numbering class, courtesy of Y_NOT (screen name), from forum: {@link
   * http://forum.java.sun.com/thread.jsp?forum=57&thread=298554}, which appears
   * to be derived from a Rule in the ScrollPane tutorial {@link
  ...
  class LineNumber extends JComponent {
    ...
    /**
     * Sets the size of this LineNumber area based on the number of digits in
     * the specified number.
     * 
     * @param number
     *          The row number to use (determines the maximum number of digits
     *          to support).
     */
    ...
    protected void paintComponent(Graphics g) {
      ...
      int startLineNumber = firstLine + 1;
      int endLineNumber = startLineNumber + (drawHere.height / lineHeight);
      int lineLocation = (((drawHere.y / lineHeight) * lineHeight) + lineHeight)
          - offset - 3;
      for (int i = startLineNumber; i <= endLineNumber; i++) {
        String lineNumber = String.valueOf(i);
        int width = fontMetrics.stringWidth(lineNumber);
        g.drawString(lineNumber, (MARGIN + currentRowWidth) - width,
            lineLocation);
  ...
  class MouseWheelHandler implements MouseWheelListener {
    public void mouseWheelMoved(MouseWheelEvent e) {
      ...
      if (adjust + line < 0) adjust = -line;
      else if (adjust + line > getLineCount()-visibleLines) adjust = getLineCount()<visibleLines?0:line-(getLineCount()-visibleLines);
