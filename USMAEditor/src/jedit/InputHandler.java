 ...
 * 
 * @author Slava Pestov
 * @version $Id: InputHandler.java,v 1.4 2004/06/09 19:15:26 dk8685 Exp $
...
public abstract class InputHandler extends KeyAdapter {
	...
	public static class insert_break implements ActionListener {
		public void actionPerformed(ActionEvent evt) {
			...
			String lineText = textArea.getText(lineStart, textArea.getSelectionStart()-lineStart);
			int numSpaces = 0;
			StringBuffer spaces = new StringBuffer();
			while (numSpaces < lineText.length()
					&& (lineText.charAt(numSpaces) == ' '
					|| lineText.charAt(numSpaces) == '\t')) {

        spaces.append(lineText.charAt(numSpaces));
				numSpaces++;
			}
	...
	/**
	 * Indents the current line by a tab; if multiple lines are selected,
	 * indents each selected line by a space.
	 * 
	...
	public static class insert_tab implements ActionListener {
		public void actionPerformed(ActionEvent evt) {
			...
			if (textArea.selectionStart == textArea.selectionEnd) {
				...
				int startLine = textArea
						.getLineOfOffset(textArea.selectionStart);
	...
	/**
	 * Unindents the current line by one tab; if multiple lines are selected,
	 * unindents each line by a single space (or removes tabs, if using tabs).
	 * 
	 * @author Karl A. Gossett
	 */
	public static class insert_shift_tab implements ActionListener {
		public void actionPerformed(ActionEvent evt) {
			...
			textArea.getDocument().beginCompoundEdit();
			if (textArea.selectionStart == textArea.selectionEnd) {
				if ("\t ".indexOf(textArea.getText(textArea.selectionStart - 1,
						1)) != -1) {
					textArea.select(textArea.selectionStart - 1,
