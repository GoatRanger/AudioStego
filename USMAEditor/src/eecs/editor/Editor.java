...
public class Editor {
  ...
  protected transient AbstractAction copyXHTMLAction;
  /** DOCUMENT ME! */
  ...
  /** Creates the startup Tips window and displays it. */
  ...
  /** Gets an instance of the system clipboard to allow cut/paste functionality. */
  ...
  /**
   *  Copies all files required to support the IT105 Wrapper and simulation environment.
   * Copies files from the network location to the local JRE lib/ext directory.
   */
  ...
  /**
   * Creates all of the menu/toolbar/shortcut actions.  All actions are an
   * instance of <code>AbstractAction</code>.
   */
  private void initActions() {
    ...
    copyXHTMLAction = new CopyXHTMLAction("Copy Formatted XHTML", //$NON-NLS-1$
        null, Messages.getString("Editor.copyxhtml.action.tooltip"), //$NON-NLS-1$ //$NON-NLS-2$
        new Integer(KeyEvent.VK_H), KeyStroke.getKeyStroke("ctrl shift H")); //$NON-NLS-1$
    copyXHTMLAction.setEnabled(false);
  ...
  /** Enables and disables the copy/cut actions in the editor.
   * 
   * @param enable   <code>true</code> if text is selected and copy/cut should be enabled. 
   */
  private void setCopyEnabled(boolean enable) {
    copyAction.setEnabled(enable);
    cutAction.setEnabled(enable);
  }

  /** Returns <code>true</code> if <strong>Copy</strong> is enabled (if text is highlighted).
   * 
   * @return <code>true</code> if the copy action is enabled.
   */
  ...
  /**
   * Sets the file chooser's filter to filter the selected file extension.
   * 
   * @param fileExt the <code>String</code> file extension.  If the filter is not one of
   * <em>.htm, .java, .css, .hud</em>, then the filter is set to all files.
   */
  private void setFileFilter(String fileExt) {
...
//        });
      ...
      if (!currentPanel.saveIfNecessary(Messages
        ...
        if (currentPanel.getSaveFile() != null) {
          messageTabs.setTitleAt(CONSOLE_TAB, Messages
              .getString("Editor.console.label") //$NON-NLS-1$
  ...
  private void clearRunProcess() {
    ...
    }, Messages.getString("Editor.182")).start(); //$NON-NLS-1$
    process = null;
  }
  ...
  private synchronized void compileAction() {
    ...
    if (canCheckForErrors()) {
      ...
      messageTabs.setSelectedIndex(CONSOLE_TAB);
      console.setText(Messages.getString("Editor.188")); //$NON-NLS-1$
      ...
      timer = new javax.swing.Timer(80, new ActionListener() {
        public void actionPerformed(ActionEvent evt) {
          ...
          if (language.getCompilerPercentComplete() == 100) {
            Toolkit.getDefaultToolkit().beep();
            timer.stop();
            // TODO: Fix the case where we get a compile complete, but no
            // messages
            //  Probably just a timing issue (check ordering of events in all
            // LanguageKits)
            int counter = 0;
  ...
  private void copyXHTMLAction() {
    ((EditPanel)programTabs.getSelectedComponent()).copyXHTMLToClipboard();
  }
  ...
  private void createConsole() {
    ...
    console.setContentType(Messages.getString("Editor.204")); //$NON-NLS-1$
  ...
  private void createEditMenu() {
    ...
    item.setIcon(null);
    menu.add(item);
    item = new JMenuItem(copyXHTMLAction);
    item.setIcon(null);
    menu.add(item);
  ...
  private void createHelpSet() {
    ...
    URL url;
    String helpSetName = "help/editor.hs"; 
    try {
      ...
      ee.printStackTrace();
      System.out.println ("Creating empty HelpSet");
      helpSet = new HelpSet();
      return;
    }
  }
  ...
  private void filesOpen(boolean oneOpen) {
    ...
    copyXHTMLAction.setEnabled(oneOpen);
  }
  ...
  private void haltAction() {
    currentProgramTab().getLanguageKit().stopRun();
//    if (process == null) {
//      return;
//    }
    if (logger.isLoggable(Level.INFO)) {
...
//    process = null;
  ...
  private void initializeGUI() {
    ...
    mainWindow.setTitle(Messages.getString("Editor.title"));
...
//          ((EditPanel) c).getProgramArea().requestFocusInWindow();
  ...
  private void initializeToolBar() {
    ...
    JComboBox<?> newBox = new JComboBox<Object>(intArray);
    ...
    button = new JButton(new ImageIcon(cl
        .getResource("eecs/editor/images/Undo24.gif")));
    button.setToolTipText(Messages.getString("Editor.undo.tooltip")); //$NON-NLS-1$
    ...
    toolBar.addSeparator();
    button = new JButton(new ImageIcon(cl
        .getResource("eecs/editor/images/Submit24.gif")));
    button.setToolTipText(Messages.getString("Editor.submit.tooltip")); //$NON-NLS-1$
    button.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        submitAction();
      }
  ...
  private void initLogging() {
    ...
    fm.deleteFilesStartingWith(homePath, logName);
    try {
  ...
  /**
   * Creates a new <code>JMenuItem</code> based on the specified parameters.
   * 
   * @param menu
   *          The <code>JMenu</code> that this item should be added to.
   * @param name
   *          The <code>String</code> text label for the new item.
   * @param keyEvent
   *          The KeyEvent that should cause this menu action to occur.
   * @param accelerator
   *          A <code>String</code> specifying the key accelerator that should activate this menu item. 
  ...
  /**
   * Creates a new editing tab of the specified type.
   * 
   * @param type
   *          One of the allowed languages.
   * 
   * @see eecs.editor.language.LanguageKit
   */
  ...
  /**
   * Opens a file in a new program tab.
   */
  private void openAction() {
    if (chooser == null) {
      updateStatus(Messages.getString("Editor.open.filechooser.error.message")); //$NON-NLS-1$
      logger.warning(Messages.getString("Editor.335")); //$NON-NLS-1$
      return;
    }
  ...
  /**
   * Opens the IT105 Course Handbook in a new browser window.
   */
...
//    }
  ...
  private void runAction() {
    ...
    if (runningPanel.getLanguageKit().isRunnable()) {
      ...
      console.setText(Messages.getString("Editor.377")
          + runningPanel.getLanguageKit().getDescription() //$NON-NLS-1$
          + Messages.getString("Editor.378")); //$NON-NLS-1$
      messageTabs.setTitleAt(CONSOLE_TAB, Messages.getString("Editor.379") //$NON-NLS-1$
          + runningPanel.getSaveFile().getName());
  ...
  class CopyXHTMLAction extends AbstractAction {
    /**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	public CopyXHTMLAction(String text, ImageIcon icon, String desc,
        Integer mnemonic, KeyStroke accel) {
      super(text, icon);
      putValue(SHORT_DESCRIPTION, desc);
      putValue(MNEMONIC_KEY, mnemonic);
  ...
  class UndoAction extends AbstractAction {
	...
	public UndoAction() {
      ...
      putValue(Action.ACCELERATOR_KEY, KeyStroke.getKeyStroke("ctrl Z")); //$NON-NLS-1$
    ...
    public void actionPerformed(ActionEvent e) {
      try {
        ...
        logger.log(Level.WARNING, "Unable to undo.", ex); //$NON-NLS-1$
      } catch (Exception ex) {
    ...
    protected void updateUndoState() {
      if (undoManager.canUndo()) {
        ...
        if (undoButton != null) {
          undoButton.setEnabled(false);
          undoButton.setToolTipText("Undo"); //$NON-NLS-1$
        }
      }
    }
  }
