...
/* adapted from the PrintListing example in the documentation
   for the Java 2D API */
import java.awt.*;
...
public class PrintText {
  public static void printText(String title, String text,
    ...
    job.setPrintable(new PrintListingPainter(title, text, lineNumbersEnabled),
      pf);
...
class PrintListingPainter implements Printable {
  ...
  public PrintListingPainter(String title, String text,
    boolean lineNumbersEnabled) {
    this.title = title;
    this.text = text;
  ...
  public int print(Graphics g, PageFormat pf, int pageIndex)
    ...
    try {
      ...
      while ((y + 12) < (pf.getImageableY() + pf.getImageableHeight())) {
        ...
        // TODO: Need to account for linewrite off bottom of page.
        while (start < line.length()) {
	        ...
	        if (end < line.length()) {
	          int tempEnd = end;
		        ...
		        if (end==start) end = tempEnd;
	        }
		      int offset= 0;
	        if (start > 0) offset += fm.stringWidth("    "+spaces);
