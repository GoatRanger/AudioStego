...
//  static final Pool stationaryThings = new Pool();
  ...
  static final double HOSPITAL_X = (Const.LEFT+Const.RIGHT) / 2.0
	                                 - 100 + 200 * Math.random();
  static final double HOSPITAL_Y = (Const.TOP+Const.BOTTOM) / 2.0
	                                 - 100 + 200 * Math.random();
 ...
 // the panel where everything is drawn
  ...
  static final Thread animationTicker = new Thread() {
    public void run() {
	  while (true) {
	    tick();
		panel.repaint();
		try { sleep(Const.TIME_BETWEEN_TICKS); }
		catch (Exception e) {}
	  }
	}
  };
  ...
  static void tick() {
	...
	for (Mobile thing = it.next(); thing != null; thing = it.next()) {
      if (thing.overlaps(hospital)) ((Robot)thing).heal();
	}
	...
	if (Clock.read() > Const.TIME_LIMIT) endGame();
  ...
  static void endGame() {
	...
	switch (robotCount) {
	  case 0:
	    ...
	    break;
      case 1:
	    Robot robot = (Robot)robots.getIterator().next();
  ...
  static void paint(Graphics g) {
	...
	if (seconds.length() == 1) seconds = "00" + seconds;
...
//static Obstacle LEFT_WALL,RIGHT_WALL,TOP_WALL,BOTTOM_WALL;
  ...
  public static void runSimulation() {
	Window.open(panel,
	            Const.RIGHT+Const.RIGHT_BORDER, 
	            Const.BOTTOM+Const.BOTTOM_BORDER,
                Const.TITLE);
