...
/**
 * <p>
 * This is the main simulation window that contains the simulation and a text area that can be used for simulation
 * (system) and user messages.
 * </p>
 * <p>
 * This class also implements the ability to start and stop the simulation via a menu option; these options should be
 * used sparingly, since they make the assumption that stopping the simulation will not adversely affect the user's
 * thread.
 * </p>
 * <p>
 * <b>Bugs: </b>
 * </p>
 * <ul>
 * <li>If the user's thread is not interacting with the simulation, the simulation will stop, but the user's thread
 * will continue, and the resulting executing might differ from an execution where the simulation wasn't stopped.</li>
 * <li>If the user's thread used a goForward or goBackward command for a robot (or other methods that return
 * immediately), execution of the user's thread will continue even though the simulation stopped. So, any assumptions
 * about time will be invalid.</li>
 * <li>Use of the System time (currentTimeMillis) will not coincide with the simulation, since simulation time will be
 * stopped.</li>
 * </ul>
 * 
 * @version 6.2.2
 * @author Karl Gossett
 */
public class SimBox extends javax.swing.JFrame implements Realism {
  ...
  /**
   * Indicates that the size of the simulation needs to change. Using this flag ensures that any changes to the
   * components will only happen in the event dispatching thread (necessary for Swing components).
   */
  ...
  /**
   * The setting for the amount of realism to use. As of version 6.3, there are 3 settings.
   */
  ...
  private eecs.robot.jago.SimBox.StreamListener updater;
   ...
   * 
   * @param width
   *          The desired width of the simulation area, in pixels
   * @param height
   *          The desired height of the simulation area, in pixels
   * @param bkgnd
   *          The name of the image to use as a background (must be a jpg or gif image)
   * @param tiled
   *          <CODE>true</CODE> if the image should be tiled to fill the simulation space. If this parameter is <CODE>
   *          false</CODE>, the simulation area will be resized to match the size of the image.
   */
  public SimBox(int width, int height, String bkgnd, boolean tiled) {
    ...
    simFrame.getContentPane().add(simEngine);
    ...
    try {
      ...
      updater = new eecs.robot.jago.SimBox.StreamListener(textInStream);
      updater.start();
    }
  ...
  /**
   * Sets the background image of the SimBox. The SimBox will use a default background if the image does not exist in
   * either the current (user) directory, or relative to the SimBox package will check both).
   * 
   * @param fileName
   *          The name of the image to load (must be either <CODE>jpg</CODE> or <CODE>gif</CODE> image)
   */
  public void setBackgroundImage(String fileName) {
    ...
    if (image != null) {
      ...
      printError("The background image named \"" + fileName + "\" could not be found. Using default.");
      setDefaultBackground();
    }
  }
   ...
   * 
   * @param img
   *          The <CODE>Image</CODE> to use as a background.
   */
  public void setBackgroundImage(java.awt.Image img) {
    simEngine.setBackgroundImage(img);
    updateSizes(new Dimension(img.getWidth(this), img.getHeight(this)));
    repaint();
  }

  /**
   * Sets the background tile of the SimBox. You can tile an image as the background by using this method. To use a
   * single image as the background, use setBackgroundImage instead. If the image cannot be found, a default background
   * will be used.
   * 
   * @param fileName
  ...
  public void setBackgroundTile(String fileName) {
    ...
    if (bkgnd != null) {
      ...
      updateSizes((int) simEngine.getPreferredSize().getWidth(), (int) simEngine.getPreferredSize().getHeight());
      repaint();
    }
    else {
      setDefaultBackground();
   ...
   * 
   * @param debug
   *          <CODE>true</CODE> if the simulation should provide debugging information. Only provides information from
   *          those classes that have implemented debugging.
   */
  ...
  /**
   * Attempts to add the SimElement to the simulation at the specified location. The usable canvas of a SimBox is 8
   * pixels narrower than its size, and about 35 pixels shorter than its height (due to the title bar).
   * 
   * @param element
   *          The element to add
   * @param xLoc
   *          The desired horizontal location
   * @param yLoc
   *          The desired vertical location
   */
  public void add(eecs.robot.jago.element.SimElement element, double xLoc, double yLoc) {
    ...
    if (!(element.inASimulation())) {
      ...
      printError("The element " + element.toString() + " was already added to the simulation.");
    }
  }

  /**
   * Adds a robot to the simulation with an initial heading. Provided for compatability with legacy Jago applications.
   * Deprecated as of version 5.1 of the simulation.
   * 
   * @param element
   *          The <CODE>StandardRobot</CODE> to add.
   * @param xLoc
   *          The desired horizontal location
   * @param yLoc
   *          The desired vertical location
   * @param heading
   *          The initial heading of the <CODE>StandardRobot</CODE>
   * @deprecated As of Version 5.1 of Jago (eecs.eecs.robot.jago)
   */
  public void add(eecs.robot.jago.element.StandardRobot element, double xLoc, double yLoc, int heading) {
   ...
   * 
   * @param listener
   *          The <CODE>ActionListener</CODE> that will act on timer events from the simengine.
   */
  ...
  /**
   * Loads a background from a file. Opens a <CODE>JFileChooser</CODE> allowing the selection of a specified
   * background (either jpg or gif).
   */
  public void loadBackground() {
    ...
    chooser.setFileFilter(new javax.swing.filechooser.FileFilter() {
      public boolean accept(java.io.File dir) {
        ...
        String filename = dir.toString().toLowerCase();

        return filename.endsWith(".jpg") || filename.endsWith(".gif") || dir.isDirectory();
      }
  ...
  /**
   * Prints a message using a "code" style of text (smaller, sansserif). The message is terminated with a newline
   * marker.
   * 
   * @param code
   *          The line of code or text message to display.
   */
  ...
  /**
   * Prints an error message in the text window using the error style. The message will be prefixed with a newline and
  ...
  /**
   * Prints a message with a terminal newline marker in the text window using the normal text style.
   * 
   * @param msg
   *          The text message to display.
   */
   ...
   */
  public void remove(eecs.robot.jago.element.SimElement element) {
  ...
  /**
   * Starts running the simualation. <br>
   * In this version, the start and stop commands are potentially dangerous because although they stop the simulation,
   * they assume that stopping the execution of the simulation will not affect the user's program, since the user thread
   * is still executing.
   */
  public void start() {
   ...
   * </p>
   * <P>
   * Any movement that relies on the system time may be corrupted if the simulation is stopped because although the
   * simulation is not running, all time-based information is based on the system clock, not on a simulation clock.
   * </p>
   */
  public void stop() {
   ...
   * 
   * @param d
   *          The new <CODE>Dimension</CODE> of the simulation. The <CODE>Dimension</CODE> should reflect the size
   *          of the actual <I>simulation </I> area, not the size of the entire Jago workspace.
   */
   ...
   * 
   * @param width
   *          The new width of the simulation
   * @param height
   *          The new height of the simulation
   */
  ...
  /**
   * Creates the <CODE>Style</CODE> s used for the different display fonts in the output window.
   */
  protected void initTextStyles() {

    //Initialize some styles.
    javax.swing.text.Style def = javax.swing.text.StyleContext.getDefaultStyleContext().getStyle(
   ...
   * 
   * @param e
   *          The <CODE>WindowEvent</CODE> that requires processing.
   */
  ...
  /**
   * If the windows need updated, this method will be called. This method should not be called by the user; it should
   * only be called from <CODE>paint</CODE>.
   */
  protected void updateWindows() {
    needsUpdated = false;

    Dimension screenSize = java.awt.Toolkit.getDefaultToolkit().getScreenSize();
    int simVertInsets = simFrame.getInsets().top + simFrame.getInsets().bottom + 28;
    int simHorizInsets = simFrame.getInsets().left + simFrame.getInsets().right;
    simFrame.setLocation(2, 2);
    simFrame.setPreferredSize(new Dimension(simEngine.getPreferredSize().width + simHorizInsets, simEngine
        .getPreferredSize().height
        + simVertInsets));

    int splitWidth = splitPane.getInsets().left + splitPane.getInsets().right + splitPane.getDividerSize()
        + simFrame.getPreferredSize().width + scrollPane.getPreferredSize().width;

    if (splitWidth > screenSize.width - 30) {
      splitWidth = screenSize.width - 30;
    }

    int splitHeight = splitPane.getInsets().top + splitPane.getInsets().bottom + simFrame.getPreferredSize().height + 2;
    simFrame.pack();
    splitPane.setPreferredSize(new Dimension(splitWidth, splitHeight));
    splitPane.setDividerLocation(simEngine.getPreferredSize().width + simHorizInsets);
    centerFrame();
    this.pack();
  }
  ...
  private void setDefaultBackground() {
    ...
    java.awt.Image icon = ImageLoader.getImage(this, "/eecs/images/white.gif");
  ...
  void apiItemActionPerformed(java.awt.event.ActionEvent evt) { 
    //
  }

  void exit(java.awt.event.ActionEvent evt) { 
    exit();
  }
  ...
  void loadBG(java.awt.event.ActionEvent evt) { //GEN-FIRST:event_loadBG
    loadBackground();
  }
   ...
   *          Empty
   */
  void musicSetting(java.awt.event.ActionEvent evt) { //GEN-FIRST:event_musicSetting
  ...
  private void print(String msg, String style) {
    ...
    try {
      outputDocument.insertString(outputDocument.getLength(), msg, outputArea.getStyle(style));
    }
    catch (javax.swing.text.BadLocationException e) {
   ...
   *          Empty
   */
  void startSimulation(java.awt.event.ActionEvent evt) { //GEN-FIRST:event_startSimulation
    start();
    startItem.setEnabled(false);
   ...
   *          Empty
   */
  void stopSimulation(java.awt.event.ActionEvent evt) { //GEN-FIRST:event_stopSimulation
    stop();
  ...
  /**
   * An inner class <CODE>Runnable</CODE> listener that monitors the <CODE>System.out</CODE> stream and forwards it
   * to the output window <CODE>Document</CODE>.
   */
      ...
      while (myThread == listenerThread) {
        ...
        try {
          ...
          if (charCount > 0) {
            ...
            javax.swing.text.Document doc = outputArea.getDocument();

            try {
              doc.insertString(doc.getLength(), new String(letters), outputArea.getStyle("system"));
            }
            catch (javax.swing.text.BadLocationException e) {
              System.err.println("Unable to insert text into " + "output area");
            }
