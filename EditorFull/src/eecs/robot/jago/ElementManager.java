...
/**
 * This class is responsible for managing the link between the SimElements in the simulation, and the SimEngine itself.
 */
public class ElementManager implements ActionListener, Realism, Runnable {

  /**
   * Performs a comparison on two SimElements based on their horizontal (x) position. This Comparator is used when
   * sorting the vector prior to detecting collisions.
   */
  static final Comparator positionCompare = new Comparator() {
    public int compare(Object o1, Object o2) {
      ...
      double xLoc = s1.getPosition().getX();
      double x2Loc = s2.getPosition().getX();
  ...
  /**
   * The list of elements that will be removed when during the next update of the simulation.
   */
  ...
  /**
   * Flag if the vector of static elements has been modified since the last check (used by the DisplayManager)
   */
  ...
  /**
   * If all elements should be removed from the simulation, this is <CODE>true</CODE>
   */
   ...
   * 
   * @param theOwner
   *          The SimEngine that this <CODE>ElementManager</CODE> updates.
   */
  ...
  /**
   * Tests if there is an element within a specified <CODE>Rectangle2D</CODE>. This method only checks for active
   * elements--if there is an element in the queue that has not been added, any overlap with that object will not be
   * detected.
   * 
   * @param region
   *          The <CODE>Rectangle2D</CODE> to look in for <CODE>SimElement</CODE>s.
  ...
  public boolean isSimElementIn(Rectangle2D region) {
    ...
    Collection tempMobile = Collections.unmodifiableCollection(mobileElements);
    Collection tempStatic = Collections.unmodifiableCollection(staticElements);
    Iterator iter;
  ...
  /**
   * Add/delete SimElements, then perform collision detection. All SimElements are responsible for updating themselves
   * based on the SimEngine timer. This method should not be called by user programs.
   * 
   * @param e
  ...
  /*
   * Perform collision detection based on rectangles. Future versions will detect against circles, and polygons.
   */

  /**
   * Determines if a collision has occurred between <CODE>SimElement</CODE> s in the simulation.
   */
  protected final void checkForCollisions() {

    /*
     * Could sort original list (might be faster overall), but introduces synchronization problems, since modifying the
     * original list. Converting to array allows for quicker access, and Arrays.sort is a native call, so fast anyway.
     */
    ...
    for (int j = 0; j < mobileArray.length; j++) {
      ...
      for (int i = j + 1; i < mobileArray.length; i++) {
        ...
        /*
         * Check for Collision. First check for possibiliy using the (bounding) shape of the element; if bounding shapes
        ...
        if (!secondRegion.isEmpty()) {
          ...
          if (collisionDetector.isCollision(firstElement, secondElement)) {
   ...
   * @return Empty
   */
  protected boolean connected(SimElement firstElement, SimElement secondElement) {
    ...
    /*
     * If both are attachments, make sure they're not attached to the same host.
     */
    if (firstAtch != null && secondAtch != null && firstAtch.attachedTo() == secondAtch.attachedTo()) {
      attached = true;
    }
   ...
   * 
   * @param first
   *          One <CODE>Collidable</CODE> element involved in a collision.
   * @param second
   *          Another <CODE>Collidable</CODE> element involved in a collision.
   */
  ...
  /**
   * Notifies elements that any pending collisions that they were involved in have been resolved.
   * 
   * @param first
   *          DOCUMENT ME!
   * @param second
   ...
   * 
   * @param first
   *          The first <CODE>Collidable</CODE> element that might collide.
   * @param second
   *          The second <CODE>Collidable</CODE> element that might collide.
   */
   ...
   * 
   * @param element
   *          The <CODE>SimElement</CODE> to add.
   * @return <CODE>true</CODE> if the element can be added.
   */
  boolean add(SimElement element) {

    if (staticElements.contains(element) || mobileElements.contains(element) || addQueue.contains(element)) {
  ...
  /**
   * Requests the removal of a <CODE>SimElement</CODE> from the simulation. Although the request is queued, the
   * element is immediately notified that is has been removed from the simulation, and its timer listener is removed
   * from the timer.
   * 
   * @param element
  ...
  private void runTask(ActionEvent e) {
    ...
    synchronized (deleteQueue) {
      ...
      while (queueElement < queueSize) {
        ...
        SimElement oldElement = (SimElement) deleteQueue.get(queueElement);
    ...
    while (mIter.hasNext()) {
      ...
      if (elem.getTimerListener() != null) {
        elem.getTimerListener().actionPerformed(e);
      }
    }
