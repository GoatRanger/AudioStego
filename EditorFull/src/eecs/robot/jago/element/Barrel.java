...
public class Barrel extends MobileElement implements Pushable {

  /**
   * If the barrel attempts to move faster than this value, it will be slowed down. Values larger than the threshold are
   * usually due to invalid calculations based on overlap or other conditions that might arise, but can be safely
   * ignored. If the barrel does not move when it it is struck by a fast moving object, try increasing this threshold.
   */
  static final double VELOCITY_THRESHOLD = 0.9;

  /**
   * Heading at next update. Used to ensure that changes to the heading only happen as a result of the action listener.
   */
  double newHeading = Double.POSITIVE_INFINITY;

  /**
   * Velocity at next update. Used to ensure that changes to velocity only happen as a result of the action listener.
   */
  ...
  public synchronized void bounceOff(Collidable collisionElement) {
    ...
    if (otherArea.isEmpty()) {
      System.err.println("ERROR: Collision reported with barrel, " + "but cannot confirm with shape intersections.");
      return;
    }
    ...
    double myCenterX = getBounds2D().getCenterX();
    double myCenterY = getBounds2D().getCenterY();
    ...
    Point2D intersectCenter = new java.awt.geom.Point2D.Double(intersection.getCenterX(), intersection.getCenterY());
    ...
    myCenter.distance(intersectCenter);
    angleFromCollider = (360 + Math.toDegrees(Math.atan2(intersectCenter.getX() - otherCenterX, otherCenterY
        - intersectCenter.getY()))) % 360;
    ...
    angleFromMe = (360 + Math.toDegrees(Math.atan2(myCenterX - intersectCenter.getX(), intersectCenter.getY()
        - myCenterY))) % 360;
    ...
    if (!(collisionElement instanceof Mobile)) {
      ...
      /*
       * If using angular velocity, then the velocity vector is at a 90 degree angle to the collision
       */
      if (usingAngular) {
      ...
      double radAngle = Math.toRadians((interimHeading + 360) - (colliderHeading % 360));
  ...
  public void draw(Graphics2D g2) {
    ...
    g2.fill(new java.awt.geom.Ellipse2D.Double(location.getX() + radius - insideRadius, location.getY() + radius
        - insideRadius, insideRadius * 2, insideRadius * 2));
  ...
  public synchronized void updatePosition() {
    ...
    double xMove = (deltaTime / 1000.0) * SIMULATION_SCALE * velVector.getMagnitude()
        * Math.sin(Math.toRadians(velVector.getDirection()));

    // yMove is negated due to reversal of y-axis in graphics system
    double yMove = -((deltaTime / 1000.0) * SIMULATION_SCALE * velVector.getMagnitude() * Math.cos(Math
        .toRadians(velVector.getDirection())));
  ...
  protected void createArea() {
    myShape = new Area(new java.awt.geom.Ellipse2D.Double(location.getX(), location.getY(), radius * 2, radius * 2));
    previousShape = (Area) myShape.clone();
    previousVel = (PolarPoint) velVector.clone();
