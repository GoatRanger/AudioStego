...
/**
 * Implements the methods for operating a Lego Mindstorms robot within the Jago Simulation. This implementation includes
 * a Light Sensor on RCX_1 because it requires it for calibration. The other two inputs (RCX_2 and 3) are left open.
 * <p>
...
public class TimingNavigatorRobot extends StandardRobot {
  ...
  /**
   * Calibrate the forward/backward movement of the robot. This requires two black lines a specified distance apart,
   * with a background as close to white as possible (it will accept many other background colors, but white will ensure
   * the best results).
   * 
   * @param inches
   *          The number of inches between the two calibration lines
   */
  public void calibrateDistance(int inches) {
    ...
    while (getSensorInput(Monitor.RCX_1) > 35) {
      //
    }
  ...
  /**
   * This simulates the rotational calibration of the robot (since rotation is already tracked via headings).
   */
   ...
   * 
   * @param distance
   *          The number of inches to back up
   */
  ...
  protected void updateDistanceMoved(double power) {
    ...
    /*
     * Determine the amount of time it is actually moving during this update. Since the movement algorithm does not
     * allow movement of less than 5ms, maintain that here.
     */
    if (timeLeftToMove > 0) {
    ...
    /*
     * Determine component distances to move based on velocity. Since MOVEMENT_SCALE is ms/inch (at a power of 10), the
     * factor based on the power (idealVelVector) is a percentage of 10 for each component.
     */
    double xMove = deltaTime / MOVEMENT_SCALE * (idealVelVector.getX() / idealVelVector.getMagnitude());
    double yMove = deltaTime / MOVEMENT_SCALE * (idealVelVector.getY() / idealVelVector.getMagnitude());

    distanceTraveled.setLocation(distanceTraveled.getX() + xMove, distanceTraveled.getY() - yMove);
  }
}