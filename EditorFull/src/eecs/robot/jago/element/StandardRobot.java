...
/**
 * Implements the methods for operating a Lego Mindstorms robot within the Jago SimBox.
 * <p>
 * This class allows a robot to be operated in both a standard (idealistic) mode and a realistic mode. Changes in
 * realistic mode:
 * </p>
 * <ul>
 * <li>The left and right motor implement "drag"; the robot may not drive straight.</li>
 * <li>Because of the drag, the robot may not pivot the exact amount specified.</li>
 * <li>The robot does not immediately halt when colliding with an object; it will continue to try to move in the
 * direction it was moving, according to the instruction given (i.e. goForward(1000) will attempt to move for 1000ms
 * regardless of any contact).</li>
 * <li>Sensors attached to the robot return values more representative of the actual sensor (if they implement realism)
 * </li>
 * </ul>
 * 
 * @author Rusl Flowers
 ...
 */
public class StandardRobot extends MobileElement implements Monitor, RobotState, eecs.robot.jago.Realism {
  /** The default file name of the robot's image */
  protected static final String defaultName = "/eecs/images/robot0.gif";
  ...
  /**
   * Prevents changing the realism once the user has set it once, or since the robot has started moving
   */
  protected boolean realismSet = false;

  /**
   * The scale of the movement of the robot, in ms/inch. The default value is 280ms/inch. For better accuracy, the robot
   * should be calibrated
   */
  protected double MOVEMENT_SCALE = 280.0;

  /**
   * The voltage reading from the robot's battery. Used in REALISTIC mode to allow functions based on the battery power
   */
  protected double batteryPower = 9.5;

  /**
   * The most recently set power level. This is equivalent to the gear on a transmission; <CODE>power</CODE> measures
   * the actual output based on this gearing.
   */
  protected double currentPowerLevel = 0;

  /**
   * The amount of resistance on the left motor; this is always 0 when in IDEALISTIC mode
   */
  ...
  /**
   * Creates an idealistic robot. This will create a robot that can have 3 sensors attached. The robot must be added to
   * a simulation before it can be used.
   */
  ...
  /**
   * Returns the distance that this element has traveled as a <code>Point2D</code>.
   * 
   * @return The total distance moved horizontally and vertically, as a Point2D
   */
  ...
  /**
   * Sets the new power of the mobile element. Power is the potential of the element, not the instantaneous velocity.
   * 
   * @param level
   *          The power setting to use.
   */
  public void setPower(double level) {
    ...
    if ((realismLevel == REALISTIC) || (realismLevel == FIXED_REALISTIC)) {
      ...
      /*
       * Using a non-linear equation to model the relationship between the power setting and speed (in m/s). The
       * regression was based on readings taken with a battery power of 7.6
       */
   ...
   * 
   * @param level
   *          A realism level from <CODE>eecs.robot.jago.Realism</CODE>
   */
  public void setRealismLevel(int level) {
    if ((level == REALISTIC) || (level == IDEALISTIC) || (level == FIXED_REALISTIC)) {
      realismLevel = level;
  ...
  /**
   * Returns the level of realism as a constant from <CODE>eecs.robot.jago.Realism</CODE>.
   * 
  ...
  /**
   * Returns the value of the sensor connected to the specified input Return SensorMonitor.NOT_CONNECTED if there is no
   * sensor, or the input is out of range.
   * 
   * @param inputNumber
   *          The input, one of <CODE>RCX_1, RCX_2, RCX_3</CODE>
   * @return The value of the input
   */
  public int getSensorInput(int inputNumber) {
    int value;

    if ((inputNumber >= 0) && (inputNumber <= inputs.length) && (inputs[inputNumber] != null)) {
   ...
   * 
   * @param input
   *          The input number to add to, one of <CODE>RCX_1, RCX_2,</CODE> or <CODE>RCX_3</CODE>
   * @param sensorType
   *          The type of sensor
   */
  ...
  /**
   * Adds the specified <CODE>StandardSensor</CODE> to this robot on the specified input port.
   * 
   * @param input
   *          The input port where the sensor should be connected
   * @param sensor
   *          A sensor to add (generally created using the <CODE>StandardSensorFactory</CODE>)
   */
  ...
  /**
   * Determines the way this object bounces off another Collidable object. Provided as part of the interface to ensure
   * elements bounce off as required
   * 
   * @param collisionElement
   *          The {@link eecs.robot.jago.element.Collidable}element to bounce off of.
   */
  ...
  /**
   * Draws this robot. Since attachments don't draw themselves (and aren't drawn by the simulation), the element to
   * which they are attached is expected to draw them.
   * 
   * @param g2
   *          The <code>Graphics2D</code> context to draw in.
   */
  ...
  /**
   * If <CODE>true</CODE> is passed, this ArtistBot will begin leaving a trail of green dots as it moves.
   * 
   * @param drawing
   *          DOCUMENT ME!
   */
  ...
  /**
   * Causes the robot to go backward. The robot will continue to travel backwards until another movement command is
   * issued (including halt).
   */
  ...
  /**
   * Causes the robot to move backward for the specified number of milliseconds or until a collision occurs, whichever
   * occurs first, and stop.
   * <p>
   * If the time to travel is less than a simulation update cycle (which is generally > 50ms), the robot will move
   * abnormally slow. This occurs because it does not take full advantage of a simulation cycle, and much of the time is
   * wasted. Although it will travel the appropriate distance (1000ms = 1000 x 1ms), it will take significantly longer
   * in the simulation.
   * </p>
   * 
   * @param milliseconds
   *          The number of milliseconds to travel
   */
  ...
  /**
   * Causes the robot to go forward. The robot will continue to travel forward until another movement command is issued.
   */
  ...
  /**
   * Causes the robot to move forward for the specified number of milliseconds or until a collision occurs, whichever
   * occurs first, and stop.
   * <p>
   * If the time to travel is less than a simulation update cycle (which is generally > 50ms), the robot will move
   * abnormally slow. This occurs because it does not take full advantage of a simulation cycle, and much of the time is
  ...
  /**
   * Stops the robot from moving, unless the robot is pivoting. If the robot is pivoting, then the robot will wait for
   * the pivot to complete and then halt.
   */
  public void halt() {
  ...
  /**
   * Performs actions when a collision occurs. The current implementation assumes that any TOUCH sensor will react to
   * any collision on the front of the robot, and that any light sensor will react only to reflections near the front
   * center of the robot.
   * 
   * @param collisionElement
   *          A collidable actor to react to
   */
  public synchronized void handleCollision(Collidable collisionElement) {
     ...
     */
    if ((collisionElement instanceof eecs.robot.jago.element.FlatObject) || (collisionElement instanceof eecs.robot.jago.element.Fire)) {
      return;
    }
    ...
    Point2D intersectCenter = new Double(intersection.getCenterX(), intersection.getCenterY());
    ...
    /*
     * Although the following sets the touch sensors ON, any touch sensor set in REALISTIC mode ignores that call, so it
     * has no impact.
     */
    ...
    if (Math.abs(diffAngle) < 10) {
      ...
      for (int i = 0; i < inputs.length; i++) {
        ...
        if ((current == RobotSensors.TOUCH) || (current == RobotSensors.INVERTED_FRONT_BUMPER)) {
          inputs[i].setValue(SensorState.ON);
        }
      }
    ...
    /*
     * Note: In REALISTIC mode, the state is not changed upon a collision. this causes the robot to continue to try to
     * move in the original direction until the command is completed.
     */
  }

  /**
   * Turns the robot the number of specified degrees to the left. The robot will stop if it is currently moving and then
   * pivot.
   * 
   * @param degrees
  ...
  public void pivotLeft(int degrees) {
    /*
     * If the robot is not currently pivoting, then pivot. Else wait for the current pivot to complete before pivoting
     * again.
     */
    if (degrees > 0) {
      lastUpdateTime = host.elapsedTimeMillis();
      pivotValue = degrees - ((rightMotorDrag * degrees) - (leftMotorDrag * degrees));
  ...
  /**
   * Pivots the robot the number of specified degrees to the right. The robot will stop if it is currently moving and
   * then pivot.
   * 
   * @param degrees
   *          The number of degrees to turn.
  ...
  public void pivotRight(int degrees) {
    /*
     * If the robot is not currently pivoting, then pivot. Otherwise, wait for the current pivot to complete before
     * pivoting again.
     */
    if (degrees > 0) {
      pivotValue = degrees - ((leftMotorDrag * degrees) - (rightMotorDrag * degrees));
  ...
  public synchronized void updatePosition() {
    ...
    double adjustment = changeFromDrag
        * (((timeLeftToMove > host.getDelay()) || (timeLeftToMove == 0)) ? host.getDelay() : timeLeftToMove);
    ...
    switch (state) {
      ...
      case PIVOTING_LEFT:
        ...
        if (pivotValue > 0) {
          /*
           * Set the amount to rotate, based on the power of the robot.
           */
  ...
  /**
   * Updates all the sensors. This method will be called by the simulation when it requires sensors to be updated.
   */
  public void updateSensors() {
    for (int inp = 0; inp < inputs.length; inp++) {
      // For now, just reset touch sensors if necessary
      if ((((state != BLOCKED) && (state != STOPPED) && (lastCommand != state)) || (state == GOING_FORWARD))
  ...
  /**
   * Causes the robot to continue what it is currently doing for a specified amount of time before executing the next
   * command.
   * 
   * @param milliseconds
   *          Specifies for how long the robot will wait
   */
  ...
  /**
   * Sets the current state of the robot. It is managed in this method to ensure synchronization, and to force the robot
   * to wait if it is pending a collision. If it is pending a collision, it will wait in this method until it is
   * notified by the completion of collision handling (see {@link eecs.robot.jago.element.MobileElement#pendingCollision}
   * 
   * @param state
  ...
  /**
   * Sets the heading of the robot to <i>degrees </i>, which is guaranteed to be between 0 and 360 degrees. If the
   * method argument is greater than 360 or less than 1, then the call is ignored.
   * 
   * @param delta
   *          Change the azimuth this many degrees.
   * @param direction
   *          PIVOTING_LEFT or PIVOTING_RIGHT
   */
  ...
  /**
   * Causes robot to continue its last instruction until <i>milliseconds </i> have passed, and then it stops.
   * 
   * @param milliseconds
  ...
  /**
   * Determines the impact that battery power has, between idealistic and realistic mode.
   * 
    ...
    for (int i = 0; i < inputs.length; i++) {
      if (inputs[i] instanceof TouchSensor) {
        if (inputs[i].getType() == RobotSensors.INVERTED_FRONT_BUMPER) {
          ...
          touchSize = Math.max(touchSize, inputs[i].getSize().getHeight());
        }
      }
    }
  ...
  void updateImage() {
    ...
    for (int i = 0; i < inputs.length; i++) {
      ...
      if ((inputs[i].getType() != RobotSensors.NO_SENSOR) && inputs[i] instanceof Sensor) {
