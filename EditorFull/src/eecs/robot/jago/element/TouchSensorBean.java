...
public class TouchSensorBean extends StandardSensor implements Serializable, Mobile {
  ...
  protected String imageName = "/eecs/images/robot_bumper_full.gif";
  ...
  public TouchSensorBean(int type) {
    ...
    if (type == INVERTED_FRONT_BUMPER) {
      imageName = "/eecs/images/robot_bumper_inverted.gif";
    }
  ...
  /**
   * Sets the picture of any object using this interface to the image identified by the URL provided.
   * 
   * @param filename
   *          The URL of the picture to use.
   */
  public void setAppearance(String filename) {
    appearance = eecs.robot.jago.ImageLoader.getImage(this, filename);
  ...
  /**
   * Determines the new power of the mobile element. In the TouchSensor, this has no meaning.
   * 
   * @param speed
   *          DOCUMENT ME!
   */
  ...
  /**
   * Sets the state of the touch sensor, if in <CODE>IDEALISTIC</CODE> mode. If in <CODE>REALISTIC</CODE> mode, this
   * method does nothing.
   * 
   * @param value
   *          The new value for the sensor (usually ON/OFF)
   * @deprecated As of Jago v6.2.3, this method is not used. The sensor now determines its own value in all modes of
   *             operation.
   */
  public synchronized void setValue(int value) {
    ...
    if (realismLevel == IDEALISTIC) {
      ...
      propertySupport.firePropertyChange("touchvalue", oldValue, this.value);
    }
  }
  ...
  /**
   * Classes must provide the ability to draw. Normally in idealistic mode the sensor will not do anything in this
   * method.
   * 
   * @param g2
   *          DOCUMENT ME!
  ...
  public void draw(java.awt.Graphics2D g2) {
    ...
    transform.rotate(Math.toRadians(heading), location.getX(), location.getY());
  ...
  /**
   * If this element has changed position during the last update, this method should return <code>true</code>.
   * 
   * @return DOCUMENT ME!
   */
  ...
  public synchronized void updatePosition() {
    ...
    if (hostMonitor instanceof Mobile) {
      ...
      double radianHeading = Math.toRadians(heading + thetaFromHostCenter);
      ...
      at.rotate(Math.toRadians(heading - oldHeading), location.getX(), location.getY());
      myShape.transform(at);

      if (!(equal(x, location.getX(), 0.5) && equal(y, location.getY(), 0.5) && equal(heading, oldHeading, 0.5))) {
        movedThisTick = true;
      }
  ...
  public void updateValue() {
    ...
    if (resetDelay > 0) {
      ...
      if (movedThisTick || (value == eecs.robot.jago.element.SensorState.NOT_READY && added)) {
        setValue(OFF);
      }
    }
  }
  ...
  /**
   * Updates the velocity of this sensor. Since it is always attached to another element, its velocity will match that
   * of its host.
   */
