...
public interface Realism {
  ...
  /**
   * Indicates that all elements should operate in a realistic manner, but should not be random each time. This allows
   * testing of a system with components acting realistically, but predictably. As an example, if a robot is created in
   * this mode, it will be created with drag on each motor, but it will always have the same drag (REALISTIC mode
   * generates a new random drag each time a robot is created).
   */
  public static final int FIXED_REALISTIC = 2;

  /**
   * Indicates that any element should react idealistically (does not adhere to the expectations of the real object)
   */
  public static final int IDEALISTIC = 1;

  /**
   * Indicates that all elements should attempt to act realistically (as close to the modeled object as possible).
   */
