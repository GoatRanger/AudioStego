...
public class ImageLoader {
  /**
   * A convenience method for loading images. This method will check for the specified resource in the following locations:
   * <ol>
   * <li>As an absolute filename specified by in <i>filename </i></li>
   * <li>Relative to the package of the requestor</li>
   * <li>Relative to the package of the requestor, but in an <code>images</code> directory</li>.
   * 
   * @param requestor
   *          The <code>URL</code> of the image
   * @param filename
   *          The name of the resource to load.
   * @return An <code>Image</code> loaded from the file. <code>null</code> if the file is not found.
   */
  public static Image getImage(Object requestor, String filename) {
    java.io.File local = new java.io.File(filename);
    java.net.URL iconURL;
    if (requestor == null) {
      requestor = new ImageLoader();
    }
    try {
      if (local.exists()) {
        try {
          iconURL = local.getAbsoluteFile().toURL();
        }
        ...
        iconURL = requestor.getClass().getResource(filename);
        if (iconURL == null) {
          // Still didn't find it, try relative to an images directory
          iconURL = requestor.getClass().getResource("images/" + filename);
        }
      }
    }
    ...
    if (iconURL != null) {
      ...
      System.err.print(requestor.getClass().getPackage().toString() + "." + filename + " and ");
      System.err.println(requestor.getClass().getPackage().toString() + ".images." + filename + ".");
    }
    if (icon != null) {
