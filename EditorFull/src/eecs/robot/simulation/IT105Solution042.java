...
public class IT105Solution042 extends IT105Project042 {
  ...
  public static void main(String[] args) {
    ...
    double x = getSatelliteXPosition() + Math.sin(Math.toRadians(p[0])) * p[1];
    double y = getSatelliteYPosition() - Math.cos(Math.toRadians(p[0])) * p[1];
    gotoPoint(x, y);
    //printLine(getMyYPosition()+"Y position");
    x = getSatelliteXPosition() + Math.sin(Math.toRadians(p[2])) * p[3];
    y = getSatelliteYPosition() - Math.cos(Math.toRadians(p[2])) * p[3];
    gotoPoint(x, y);
    findBlackLine();
    ...
    /*
     * int[] p=new int[4]; p=getPolarCoordinates(); for (int i=0;i <4;i++){ printLine(p[i]); }
     * printLine(Math.cos(Math.toRadians(p[0]))*p[1]+"y value"); printLine(Math.sin(Math.toRadians(p[0]))*p[1]+"x value"); double
     * x=500-Math.sin(Math.toRadians(p[0]+180))*p[1]; double y=10-Math.cos(Math.toRadians(p[0]))*p[1]; printLine(x+" x "+y+ " y");
     * gotoPoint(x,y); x=500-Math.sin(Math.toRadians(p[2]+180))*p[3]; y=10-Math.cos(Math.toRadians(p[2]))*p[3]; gotoPoint(x,y);
     * transmitMissionComplete();
     */
  }
}