...
class Const {
  ...
  static final double[] START_X = new double[] { LEFT + OFFSET, RIGHT - OFFSET, (LEFT + RIGHT) / 2, (LEFT + RIGHT) / 2 };
  static final double[] START_Y = new double[] { (TOP + BOTTOM) / 2, (TOP + BOTTOM) / 2, TOP + OFFSET, BOTTOM - OFFSET };
  static final double[] START_HEADING = new double[] { EAST, WEST, SOUTH, NORTH };
  // shapes
  /*
   * static final Shape MISSILE_EXPLOSION_SHAPE = Util.circle( MISSILE_EXPLOSION_RADIUS); static final Shape ROBOT_EXPLOSION_SHAPE =
   * Util.circle( ROBOT_EXPLOSION_RADIUS); static final Shape[] WALL_SHAPES = new Shape[] { Util.rectangle(LEFT - WALL_WIDTH / 2, (TOP +
   * BOTTOM) / 2.0, WALL_WIDTH, BOTTOM - TOP + 2 * WALL_WIDTH), Util.rectangle(RIGHT + WALL_WIDTH / 2, (TOP + BOTTOM) / 2.0, WALL_WIDTH,
   * BOTTOM - TOP + 2 * WALL_WIDTH), Util.rectangle((LEFT + RIGHT) / 2.0, TOP - WALL_WIDTH / 2, RIGHT - LEFT + WALL_WIDTH, WALL_WIDTH),
