/*
 * Copyright (C) 2002 USMA This program is free software; you can redistribute it and/or modify it under the terms of the GNU General Public
...
class Baseball {
  ...
  public static BaseballTeam Home = null;
  public static BaseballTeam Visitor = null;
  ...
  private static String[] chooseRobots(String[] candidates) {
    ...
    // build the gui
    JComboBox[] player = new JComboBox[] { new JComboBox(choices), new JComboBox(choices) };
    /*
     * set the first two selections randomly (duplicates okay) player[0].setSelectedIndex( 1 + (int)(Math.random() * candidates.length));
     * player[1].setSelectedIndex( 1 + (int)(Math.random() * candidates.length));
     */
    String RUN = "Play Ball";
    int value = JOptionPane.showOptionDialog(null, new Object[] { "Visitors   ", player[0], "Home   ", player[1] // message
        }, "Choose Teams", // title,
        JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, // icon
        new String[] { RUN, "Quit" }, RUN // default item
        );
    switch (value) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      if (name == null || name == NONE) {
        ...
        /*
         * if (i==0) Visitor.setName(names[i]); else Home.setName(names[i]);
         */
        numPicked++;
      }
    }
  ...
  private static String[] findCandidates() {
    ...
    for (int i = 0; i < files.length; i++) {
      ...
      candidates[i] = filename.substring(Const.PREFIX.length(), filename.length() - Const.SUFFIX.length());
    }
    return candidates;
  } // end findCandidates
    ...
    for (int i = 0; i < names.length; i++) {
      if (names[i] != null) {
        ...
        try {
          ...
          if (i != 1) {
            Visitor = (BaseballTeam) teamClass.newInstance();
          }
          else {
            Home = (BaseballTeam) teamClass.newInstance();
          }
  ...
  public static BaseballTeam getHomeTeam() {
    return Home;
  }

  public static BaseballTeam getVisitingTeam() {
    return Visitor;
  }
