/*
 * TokenMarker.java - Generic token marker Copyright (C) 1998, 1999 Slava Pestov You may use and modify this package for any purpose.
 ...
 * 
 * @author Slava Pestov
 * @version $Id: TokenMarker.java,v 1.1 2005/01/07 16:17:16 dj2087 Exp $
 * @see eecs.editor.jedit.Token
 */
public abstract class TokenMarker {
  ...
  public Token markTokens(Segment line, int lineIndex) {
    ...
    info.token = token;
    /*
     * This is a foul hack. It stops nextLineRequested from being cleared if the same line is marked twice. Why is this necessary? It's all
     * JEditTextArea's fault. When something is inserted into the text, firing a document event, the insertUpdate() method shifts the caret
     * (if necessary) by the amount inserted. All caret movement is handled by the select() method, which eventually pipes the new position
     * to scrollTo() and calls repaint(). Note that at this point in time, the new line hasn't yet been painted; the caret is moved first.
     * scrollTo() calls offsetToX(), which tokenizes the line unless it is being called on the last line painted (in which case it uses the
     * text area's painter cached token list). What scrollTo() does next is irrelevant. After scrollTo() has done it's job, repaint() is
     * called, and eventually we end up in paintLine(), whose job is to paint the changed line. It, too, calls markTokens(). The problem was
     * that if the line started a multiline token, the first markTokens() (done in offsetToX()) would set nextLineRequested (because the
     * line end token had changed) but the second would clear it (because the line was the same that time) and therefore paintLine() would
     * never know that it needed to repaint subsequent lines. This bug took me ages to track down, that's why I wrote all the relevant info
  ...
  /**
   * Returns true if the next line should be repainted. This will return true after a line has been tokenized that starts a multiline token
  ...
  public class LineInfo {
    ...
    public byte token;
    /**
     * This is for use by the token marker implementations themselves. It can be used to store anything that is an object and that needs to
