/*
 * InputHandler.java - Manages key bindings and executes actions Copyright (C) 1999 Slava Pestov You may use and modify this package for any
 ...
 * 
 * @author Slava Pestov
 * @version $Id: InputHandler.java,v 1.1 2005/01/07 16:17:16 dj2087 Exp $
 * @see eecs.editor.jedit.DefaultInputHandler
 */
public abstract class InputHandler extends KeyAdapter {
  /**
   * If this client property is set to Boolean.TRUE on the text area, the home/end keys will support 'smart' BRIEF-like behaviour (one press =
   * start/end of line, two presses = start/end of viewscreen, three presses = start/end of document). By default, this property is not set.
   */
   ...
   * 
   * @param listener
   *          The <code>ActionListener</code> to get a keystroke from.
   */
  ...
  /**
   * Indents the current line by a tab; if multiple lines are selected, indents each selected line by a space.
   * 
  ...
  public static class insert_tab implements ActionListener {
    public void actionPerformed(ActionEvent evt) {
      ...
      if (textArea.selectionStart == textArea.selectionEnd) {
        ...
        int startLine = textArea.getLineOfOffset(textArea.selectionStart);
        int endLine = textArea.getLineOfOffset(textArea.selectionEnd);
        for (int i = endLine; i >= startLine; i--) {
          int offset = textArea.getLineStartOffset(i);
          textArea.select(offset, offset);
          textArea.overwriteSetSelectedText(" ");
        }
        textArea.select(start + 1, end + (endLine - startLine) + 1);
      }
  ...
  /**
   * Unindents the current line by one tab; if multiple lines are selected, unindents each line by a single space (or removes tabs, if using
   * tabs).
   * 
   * @author Karl A. Gossett
   */
  public static class insert_shift_tab implements ActionListener {
    public void actionPerformed(ActionEvent evt) {
      ...
      textArea.getDocument().beginCompoundEdit();
      if (textArea.selectionStart == textArea.selectionEnd) {
        if ("\t ".indexOf(textArea.getText(textArea.selectionStart - 1, 1)) != -1) {
        ...
        int endLine = textArea.getLineOfOffset(textArea.selectionEnd);
        int changes = 0;
        int firstLineChange = 0;
        for (int i = endLine; i >= startLine; i--) {
          int offset = textArea.getLineStartOffset(i);
          textArea.select(offset, offset + 1);
          if (" \t".indexOf(textArea.getSelectedText()) != -1) {
            textArea.overwriteSetSelectedText("");
            if (i == startLine)
              firstLineChange = 1;
            changes++;
        ...
        textArea.select(start - firstLineChange, end - changes);
      }
      textArea.getDocument().endCompoundEdit();
    }
  }
