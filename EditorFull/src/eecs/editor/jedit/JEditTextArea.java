// JEditTextArea.java - jEdit's text component Copyright (C) 1999 Slava Pestov You may use and modify this package for any purpose.
// Redistribution is permitted, in both source and binary form, provided that this notice remains intact in all source distributions of this
// package.
...
paste * 
 * @author Slava Pestov
 * @version $Id: JEditTextArea.java,v 1.2 2005/01/18 17:29:43 dj2087 Exp $
 */
public class JEditTextArea extends JComponent {
  /**
   * Adding components with this name to the text area will place them left of the horizontal scroll bar. In jEdit, the status bar is added
   * this way.
   */
  ...
  public JEditTextArea(TextAreaDefaults defaults) {
    ...
    lineNumbering = new LineNumber(this);
    add(GUTTER, lineNumbering);
    add(CENTER, painter);
    //add(CENTER,jpl);
    ...
    painter.addMouseMotionListener(mh);
  ...
  public void setFirstLine(int firstLine) {
    ...
    if (firstLine != vertical.getValue())
      updateScrollBars();
    painter.repaint();
    lineNumbering.setFirstLine(firstLine);
   ...
   * 
   * @param file
   *          The <code>File</code> that specifies which file to load.
   * @throws IOException
   ...
   * 
   * @param file
   *          The <code>File</code> object where the document should be saved.
   */
   ...
   * This is a fast version that should only be used if no changes were made to the text
  ...
  public void setTextSize(int points) {
    textSize = points;
    painter.setFont(new Font("monospaced", Font.PLAIN, textSize));
    painter.invalidateLineRange(0, getLineCount());
    lineNumbering.setFont(new Font("monospaced", Font.PLAIN, textSize));
    updateScrollBars();
    painter.repaint();
    lineNumbering.repaint();
    //validate();
  ...
  // protected members
  protected static String GUTTER = "gutter";
  ...
  protected LineNumber lineNumbering;
  ...
  /**
   * Find the next occurrence of the specified search string. Uses regular expression matching.
   * 
   * @param searchFor
   *          A String pattern to search for (regex)
   * @param isCaseSensitive
   *          A boolean that is <code>true</code> if the search should be case sensitive.
   * @param wholeWordOnly
   *          A boolean that is <code>true</code> if the search should only match whole words.
   * @return <code>true</code> if the search pattern was found.
   */
  public boolean find(String searchFor, boolean isCaseSensitive, boolean wholeWordOnly) {
    boolean found = false;
    int flags;
    int startPos = getCaretPosition() + 1;
    if (isCaseSensitive) {
      flags = 0;
    }
    else {
  ...
  /**
   * Find a substring in the document and replace it with the specified text.
   * 
   * @param searchPattern
   *          The String of text to find
   * @param replace
   *          The String to replace with
   * @param isCaseSensitive
   *          a boolean indicating if case should be used in the search
   * @param matchWholeWords
   *          a boolean indicating if the search should only replace when the searchPattern matches a complete word (not a substring)
   * @return <code>true</code> if a replacement was made.
   * @see #find
   */
  public boolean replace(String searchPattern, String replace, boolean isCaseSensitive, boolean matchWholeWords) {
    boolean success = false;
    if (find(searchPattern, isCaseSensitive, matchWholeWords)) {
      setSelectedText(replace);
      success = true;
    }
    return success;
  }
   ...
   *          The new text
   * @param isCaseSensitive
   *          <code>true</code> if the replacement is case sensitive
   * @param matchWholeWords
   *          <code>true</code> only if the pattern should only match whole words
   * @return <code>true</code> if any replacements were made
   */
  public boolean replaceAll(String searchPattern, String replace, boolean isCaseSensitive, boolean matchWholeWords) {
  ...
  class ScrollLayout implements LayoutManager {
    ...
    public Dimension preferredLayoutSize(Container parent) {
      ...
      Dimension gutterPref = gutter.getPreferredSize();
      dim.width += gutterPref.width;
    ...
    public Dimension minimumLayoutSize(Container parent) {
      ...
      dim.width = insets.left + insets.right;
      dim.height = insets.top + insets.bottom;
      Dimension gutterPref = gutter.getMinimumSize();
      dim.width += gutterPref.width;
    ...
    public void layoutContainer(Container parent) {
      ...
      int gutterWidth = gutter.getPreferredSize().width;
      int rightWidth = right.getPreferredSize().width;
      int bottomHeight = bottom.getPreferredSize().height;
      int centerWidth = size.width - gutterWidth - rightWidth - ileft - iright;
      int centerHeight = size.height - bottomHeight - itop - ibottom;
      gutter.setBounds(ileft, itop, gutterWidth, centerHeight);
      center.setBounds(ileft + gutterWidth, itop, centerWidth, centerHeight);
      right.setBounds(ileft + gutterWidth + centerWidth, itop, rightWidth, centerHeight);
  ...
  class LineNumber extends JComponent {
    private static final int HEIGHT1 = Integer.MAX_VALUE - 1000000;
    // Set right/left margin
    private static final int MARGIN = 5;
    private final Color DEFAULT_BACKGROUND = new Color(228, 228, 228);
    private final Color DEFAULT_FOREGROUND = Color.red.darker();
    private final Font DEFAULT_FONT = new Font("Monospaced", Font.PLAIN, 12);
    // Metrics of this LineNumber component
    private FontMetrics fontMetrics;
    private int currentRowWidth;
    private int firstLine1 = 0;
     ...
     * 
     * @param component
     *          The JComponent that this LineNumber supports.
     */
    public LineNumber(JComponent component) {
      ...
      if (component == null) {
        ...
        setFont(painter.getFont());
      }
      setPreferredSize(maxRow);
    }

    /**
     * Sets the font of the line numbering area.
     * 
     * @param font
     *          The Font to use.
     */
    public final void setFont(Font font) {
      ...
      offset = fontLineHeight / 10;
      setPreferredSize(maxRow);
    }

    /**
     * Sets the size of this LineNumber area based on the number of digits in the specified number.
     * 
     * @param number
     *          The row number to use (determines the maximum number of digits to support).
     */
    public final void setPreferredSize(int number) {
      int width = fontMetrics.stringWidth(String.valueOf(number));
      setPreferredSize(new Dimension((2 * MARGIN) + width, HEIGHT1));
      currentRowWidth = width;
      maxRow = number;
    }

    /**
     * The line height is the height of the font for this component.
     * 
     * @return DOCUMENT ME!
     */
    ...
    protected void paintComponent(Graphics g) {
      ...
      int startLineNumber = firstLine1 + 1;
      int endLineNumber = startLineNumber + (drawHere.height / lineHeight);
      int lineLocation = (((drawHere.y / lineHeight) * lineHeight) + lineHeight) - offset - 3;
      for (int i = startLineNumber; i <= endLineNumber; i++) {
        String lineNumber = String.valueOf(i);
        int width = fontMetrics.stringWidth(lineNumber);
        g.drawString(lineNumber, (MARGIN + currentRowWidth) - width, lineLocation);
  ...
  class MouseWheelHandler implements MouseWheelListener {
    public void mouseWheelMoved(MouseWheelEvent e) {
      // Adjust 2 lines for every scroll
      int adjust = e.getWheelRotation() * 2;
      JEditTextArea textArea = InputHandler.getTextArea(e);
      int line = textArea.getFirstLine();
      if (adjust + line < 0)
        adjust = -line;
      else
        if (adjust + line > getLineCount() - visibleLines)
          adjust = getLineCount() < visibleLines ? 0 : line - (getLineCount() - visibleLines);
      textArea.setFirstLine(line + adjust);
    }
  }
  ...
  class MouseHandler extends MouseInputAdapter {
    ...
    private void doDoubleClick(MouseEvent evt, int line, int offset, int dot) {
       ...
       * TextUtilities.findWordStart(lineText,offset,noWordSep); int wordEnd = TextUtilities.findWordEnd(lineText,offset,noWordSep); int
       * lineStart = getLineStartOffset(line); select(lineStart + wordStart,lineStart + wordEnd);
       */
    }
