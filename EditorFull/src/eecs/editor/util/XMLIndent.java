...
public class XMLIndent {
  ...
  StringBuffer buffer;
  int tabSize = 2;
  int resultCode = LanguageKit.NOT_SUPPORTED;
  String[] selfClosingList = { "img", "link", "br", "hr", "META" };
  // title and a are added to the inline to make formatting look nicer
  String[] inlineElements = { "a", "abbr", "acronym", "b", "big", "cite", "code", "dfn", "em", "kbd", "q", "samp", "small", "span",
      "strong", "sub", "sup", "title", "var", "i" };
  DefaultHandler handler = new FormatHandler();

  /**
   * Sets the number of spaces to indent for a tab (each indent level is a 'tab') Values < 1 default to 0; maximum is 10.
   * 
   * @param spaces
    ...
    switch (type) {
      ...
      case Node.ELEMENT_NODE: {
        ...
        for (int loopIndex = 0; loopIndex < attributes.length; loopIndex++) {
          ...
          if (attribute.getNodeName().equals("xmlns") && !node.getNodeName().equals("html")) {
            continue;
          }
          buffer.append(" ");
        ...
        if (childNodes != null) {
          ...
          if ((length == 1) && (childNodes.item(0).getNodeType() == Node.TEXT_NODE)) {
            hadOnlyText = true;
          }
        }
        break;
  ...
  class FormatHandler extends DefaultHandler {
    String indentString = "  ";
    /**
     * The current indent level of the document Initially set to -1 so xml header and html tag stay on same level (assumes xml tag)
     */
    int indentLevel = -1;
    String lastTag = "";
