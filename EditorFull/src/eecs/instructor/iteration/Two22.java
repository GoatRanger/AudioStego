/*
 * Copyright (C) 2003 USMA This program is free software; you can redistribute it and/or modify it under the terms of
...
/**
 * GUI for writing games that play the game of Nim. This game uses marbles. The setInitialSize() method sets the initial
 * size of the pile of marbles. The pile must be between 1 - 100 marbles. Players may remove 1 or no more than half of
 * the marbles on their turn. The person to draw the last marble loses.
 */
public class Two22 extends eecs.RobotGuiBase {
  private static final int MAX_DOWN_CARDS = 250;

  static {
    sim = new eecs.robot.jago.SimBox();
    sim.setSimTitle("2-22");

    Image bg = ImageLoader.getImage(Two22.class, "/eecs/images/cobb/images/7-27.GIF");
  ...
  public static boolean doBothPlayersHaveMoney() {
    return ((humanPlayer.getBank() > 0) & (dealer.getBank() > 0));
  }
  ...
  /**
   * deals the first two cards to both players with one card dealt down to the dealer. Display is updated (round, card
   * totals, etc)
   */
  public static void initializeNewRound() {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      /*
       * sim.add(cards[idx], humanPlayer.getCurCardPositionX(),230); printLine("***just added card "+idx);
       * humanPlayer.addtoCardTotal(cards[idx].value); idx++;
       */
      givePlayerACard();
    ...
    updateDisplay();
    dealer.setCurCardPositionX(dealer.getCurCardPositionX() - 15); //this allows the face down card to be covered on
    // dealer's turn
  }

  /**
   * Sets the initial size of the pile of marbles. May only be called once. Calling more than once will cause an
   * IllegalStateException to be thrown.
   * 
   * @param size
   *          The initial size of the pile (1 - 100)
   */
   ...
   * 
   * @param number
   *          The number to remove from the pile
   */
  ...
  /**
   * Removes marbles from the pile. The number must be between 1 and 100, and it cannot be more than half of the
   * remaining marbles on the pile.
   * 
   * @param number
   *          The number to remove from the pile
   */
  ...
  /*
   * public static void removeMarble(int number) { if ((number < 0) || ((marbleCount > 1) && (number > (marbleCount /
   * 2)))) { throw new IllegalArgumentException( "Illegal argument was passed into " + "removeMarble: the argument must
   * be greater than zero and it cannot " + "be greater than half of the marbles left on the pile."); } for (int i = 1;
   * (i <= number) && (marbleCount > 0); i++) { sim.remove(marbles[marbleCount - 1]); marbleCount--; } }
   */

  /**
   * Clears the display of marbles. The setInitialSize() method may be called again after this method has been called.
   */
  ...
  private static void createCards() {
    int randomIndex;
    String randomSuit;
    String[] deck = { "k", "a", "2", "3", "4", "5", "6", "7", "8", "9", "10", "j", "q" };
    String[] suit = { "c", "d", "h", "s" };
  ...
  /*
   * private static void createMarbles() { if (!marblesCreated) { for (int i = 0; i < initSize; i++) { if ((i % 2) == 0) {
   * marbles[i] = new Marble(Marble.RED); } else if ((i % 3) == 0) { marbles[i] = new Marble(Marble.BLUE); } else {
   * marbles[i] = new Marble(Marble.YELLOW); } } marbles[3]=new Marble(Marble.down); marblesCreated = true; } }
   */
}