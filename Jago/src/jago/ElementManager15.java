...
public class ElementManager15
    ...
    protected List<SimElement> addQueue = new ArrayList<SimElement>();

    /**
     * The list of elements that will be removed when during the next update of
     * the simulation.
     */
    protected List<SimElement> deleteQueue = new ArrayList<SimElement>();
     ...
     */
    protected Collection<SimElement> lightSources = new ArrayList<SimElement>();

    /** The list of mobile elements currently alive in the sim */
    protected Collection<SimElement> mobileElements = new ArrayList<SimElement>();
    /** The list of static (nonmobile) elements currently alive. */
    protected Collection<SimElement> staticElements = new ArrayList<SimElement>();
    ...
    /**
     * If all elements should be removed from the simulation, this is <CODE>
     * true</CODE>
     */
     ...
     */
    public ElementManager15(SimEngine theOwner) {
        owner = theOwner;
    }
    ...
    /**
     * Tests if there is an element within a specified <CODE>Rectangle2D
     * </CODE>. This method only checks for active elements--if there is an
     * element in the queue that has not been added, any overlap with that
     * object will not be detected.
     * 
     * @param region
     *          The <CODE>Rectangle2D</CODE> to look in for <CODE>SimElement
     *          </CODE>s.
    ...
    public boolean isSimElementIn(Rectangle2D region) {
        ...
        for (Object anElement : mobileElements) {
            // If a visible element, see if it intersects
            if (anElement instanceof Collidable) {

                Area otherRegion = ((Collidable)anElement).getArea();
        ...
        if (!overlapExisting) {
          ...
          for (Object anElement : staticElements) {
                // If a visible element, see if it intersects
                if (anElement instanceof Collidable) {
    ...
    /**
     * Add/delete SimElements, then perform collision detection. All SimElements
     * are responsible for updating themselves based on the SimEngine timer.
     * This method should not be called by user programs.
     * 
     * @param e
    ...
    /**
     * Determines if a collision has occurred between <CODE>SimElement</CODE>
     * s in the simulation.
     */
    protected final void checkForCollisions() {
         ...
         * Converting to array allows for quicker access, and Arrays.sort is a
         * native call, so fast anyway.
         */
        ...
        for (int j = 0; j < mobileArray.length; j++) {
            ...
            /*
             * TODO: In v6.2.3, this is still required to ensure that sensors in
             * IDEALISTIC mode will reset (generally they set automatically but
             * will keep the "set" state until directed to change by this method
             * call). Need to update the sensors or the Monitor to improve this
             * interface.
             */
            ...
            for (int i = j + 1; i < mobileArray.length; i++) {
                ...
                /*
                 * Check for Collision. First check for possibiliy using the
                 * (bounding) shape of the element; if bounding shapes overlap,
                 * check pixels in handleCollision.
                 */
    ...
    protected boolean connected(SimElement firstElement,
        ...
        /*
         * TODO: Fix attachments so they never report collisions with the
        ...
        /*
         * If both are attachments, make sure they're not attached to the same
         * host.
         */
     ...
     * @param second
     *          Another <CODE>Collidable</CODE> element involved in a
     *          collision.
     */
    ...
    /**
     * Notifies elements that any pending collisions that they were involved in
     * have been resolved.
     * 
     * @param first
     *          DOCUMENT ME!
     ...
     * Although the request is queued, the element is immediately notified that
     * is has been removed from the simulation, and its timer listener is
    ...
    private void runTask(ActionEvent e) {
        ...
        synchronized (deleteQueue) {
            ...
            while (queueElement < queueSize) {
                ...
                //Remove this if statement if controlling elements from the
				// ElementManager
         ...
         * collision detection (in the ElementManager) and element updates can
         * occur simultaneously, the current implementation does not handle
         * collisions well (it essentially ignores them, since the element's
         * positions are updated before collision detection can notify them that
         * they shouldn't change position).
         */
