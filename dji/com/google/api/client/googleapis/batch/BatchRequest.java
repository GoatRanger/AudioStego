...
public final class BatchRequest {
    ...
    public void execute() throws IOException {
        ...
        while (true) {
            ...
            boolean z = i > 0;
            BatchUnparsedResponse multipartContent = new MultipartContent();
            multipartContent.getMediaType().setSubType("mixed");
            int i2 = 1;
            for (RequestInfo requestInfo : this.requestInfos) {
                int i3 = i2 + 1;
                multipartContent.addPart(new Part(new HttpHeaders().setAcceptEncoding(null).set("Content-ID", Integer.valueOf(i2)), new HttpRequestContent(requestInfo.request)));
                i2 = i3;
            }
            buildPostRequest.setContent(multipartContent);
            HttpResponse execute = buildPostRequest.execute();
            try {
                multipartContent = new BatchUnparsedResponse(execute.getContent(), "--" + execute.getMediaType().getParameter("boundary"), this.requestInfos, z);
                while (multipartContent.hasNext) {
                    multipartContent.parseNextResponse();
                }
                List list = multipartContent.unsuccessfulRequestInfos;
                if (!list.isEmpty()) {
                    this.requestInfos = list;
                    if (multipartContent.backOffRequired && backOffPolicy != null) {
