...
public class bb {
    ...
    public static LatLng a(Context context, LatLng latLng) {
        ...
        String a = du.a(context, "libwgs2gcj.so");
    ...
    private static DPoint a(DPoint dPoint, boolean z) {
        ...
        try {
            dArr2 = new double[2];
            if (z) {
                if (CoordUtil.convertToGcj(new double[]{dPoint.x, dPoint.y}, dArr2) != 0) {
                    dArr2 = gd.a(dPoint.x, dPoint.y);
                }
                dArr = dArr2;
            } else {
                dArr = gd.a(dPoint.x, dPoint.y);
            }
        } catch (Throwable th) {
            return dPoint;
        }
    ...
    public static LatLng b(Context context, LatLng latLng) {
        try {
            ...
            latLng = a(context, new LatLng(c.y, c.x, false));
        } catch (Throwable th) {
            th.printStackTrace();
        }
        return latLng;
    ...
    private static DPoint c(double d, double d2) {
        ...
        return new DPoint(((double) i2) / 100000.0d, ((double) ((int) (((double) (d4 > 0.0d ? 1 : -1)) + ((-b((double) i2, (double) i)) + d4)))) / 100000.0d);
    }
