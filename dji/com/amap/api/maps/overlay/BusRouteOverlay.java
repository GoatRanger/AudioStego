...
public class BusRouteOverlay extends b {
    ...
    private void a(LatLng latLng, String str, String str2) {
        addStationMarker(new MarkerOptions().position(latLng).title(str).snippet(str2).anchor(d.c, d.c).visible(this.nodeIconVisible).icon(getWalkBitmapDescriptor()));
    }
    ...
    private void b(RouteBusLineItem routeBusLineItem) {
        ...
        addStationMarker(new MarkerOptions().position(a).title(busLineName).snippet(c(routeBusLineItem)).anchor(d.c, d.c).visible(this.nodeIconVisible).icon(getBusBitmapDescriptor()));
    }
