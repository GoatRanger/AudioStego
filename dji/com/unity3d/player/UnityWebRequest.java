...
class UnityWebRequest implements Runnable {
    ...
    public void run() {
        try {
            ...
            if (url.getProtocol().equalsIgnoreCase(d.A) && !url.getHost().isEmpty()) {
                ...
                if ((headerFields == null || !headerFields.containsKey(com.alipay.sdk.f.d.d)) && openConnection.getContentType() != null) {
                    headerCallback(com.alipay.sdk.f.d.d, openConnection.getContentType());
                }
                uploadCallback = openConnection.getContentLength();
                if (uploadCallback > 0) {
                    contentLengthCallback(uploadCallback);
                }
                if (url.getProtocol().equalsIgnoreCase(d.A)) {
