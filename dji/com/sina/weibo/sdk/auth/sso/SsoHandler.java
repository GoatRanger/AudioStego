...
public class SsoHandler {
        ...
        if (i == this.mSSOAuthRequestCode) {
            if (i2 == -1) {
                if (SecurityHelper.checkResponseAppLegal(this.mAuthActivity, this.mWeiboInfo, intent)) {
                    ...
                    if (stringExtra == null) {
                        stringExtra = intent.getStringExtra(ab.al);
                    }
                    if (stringExtra == null) {
                        ...
                        String stringExtra2 = intent.getStringExtra(ab.am);
                ...
                if (intent != null) {
                    ...
                    this.mAuthListener.onWeiboException(new WeiboDialogException(intent.getStringExtra("error"), intent.getIntExtra(ab.an, -1), intent.getStringExtra("failing_url")));
                    return;
                }
                LogUtil.d(TAG, "Login canceled by user.");
                this.mAuthListener.onCancel();
            ...
            if (i2 == -1) {
                ...
                if (intent != null) {
                    ...
                    if (stringExtra != null) {
                        this.mAuthListener.onWeiboException(new WeiboDialogException(intent.getStringExtra("error"), intent.getIntExtra(ab.an, -1), intent.getStringExtra(ab.am)));
                        return;
                    }
                    return;
                }
    ...
    public static ComponentName isServiceExisted(Context context, String str) {
        for (RunningServiceInfo runningServiceInfo : ((ActivityManager) context.getSystemService(Constants.FLAG_ACTIVITY_NAME)).getRunningServices(Integer.MAX_VALUE)) {
