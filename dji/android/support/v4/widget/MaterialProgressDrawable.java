...
class MaterialProgressDrawable extends Drawable implements Animatable {
    ...
    private void updateRingColor(float f, Ring ring) {
        if (f > 0.75f) {
            ring.setColor(evaluateColorChange((f - 0.75f) / c.a, ring.getStartingColor(), ring.getNextColor()));
        }
    }
        ...
        Animation anonymousClass1 = new Animation() {
            public void applyTransformation(float f, Transformation transformation) {
                ...
                if (f <= d.c) {
                    ...
                    ring.setStartTrim(startingStartTrim + (MaterialProgressDrawable.MATERIAL_INTERPOLATOR.getInterpolation(f / d.c) * f2));
                }
                if (f > d.c) {
                    ring.setEndTrim(((MaterialProgressDrawable.MAX_PROGRESS_ARC - access$100) * MaterialProgressDrawable.MATERIAL_INTERPOLATOR.getInterpolation((f - d.c) / d.c)) + startingEndTrim);
                }
                ring.setRotation((c.a * f) + startingRotation);
