...
public class FileProvider extends ContentProvider {
    ...
    private static final File DEVICE_ROOT = new File(d.t);
    ...
    static class SimplePathStrategy implements PathStrategy {
        ...
        public Uri getUriForFile(File file) {
            try {
                ...
                if (path2.endsWith(d.t)) {
                    path = canonicalPath.substring(path2.length());
                } else {
                    path = canonicalPath.substring(path2.length() + 1);
                }
                return new Builder().scheme("content").authority(this.mAuthority).encodedPath(Uri.encode((String) entry.getKey()) + '/' + Uri.encode(path, d.t)).build();
            } catch (IOException e) {
