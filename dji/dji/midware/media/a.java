...
public class a implements OnAudioFocusChangeListener {
    ...
    public void a(MediaFormat mediaFormat) {
        if (!this.q) {
            ...
            e.c(false, b, "csd-0=" + c.i(dji.thirdparty.afinal.c.c.a(byteBuffer.array(), byteBuffer.arrayOffset(), byteBuffer.limit() - byteBuffer.position())));
            try {
            ...
            this.l.start();
            this.m = new AudioTrack(3, valueOf.intValue(), 4, 2, 16384, 1);
            this.m.play();
        }
    }

    public void a(ByteBuffer byteBuffer, int i, int i2, long j) {
        e.c(false, b, "test 1 ptsUs=" + ((1000 * j) / 90));
        try {
            if (this.j != null) {
                this.j.write(byteBuffer.array(), i, i2);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        if (!this.q) {
            if (this.p != a.ADTS) {
                ...
                byteBuffer.get(bArr);
                e.c(false, b, "Frame head=" + c.i(bArr));
                i3 = ((bArr[2] & 192) >>> 6) + 1;
                int i4 = k[(bArr[2] & 60) >> 2];
                int i5 = ((bArr[3] & 192) >>> 6) | ((bArr[2] & 1) << 2);
                e.c(false, b, "profile=" + i3 + " sampleRate=" + i4 + " channel=" + i5);
                a(i4, i5, i3);
            }
        }
        boolean z = false;
        ...
        if (i3 >= 0) {
            ...
            e.c(false, b, "test 3");
            while (i5 >= 0) {
                e.c(false, b, "decoder outputs " + this.n.size + " bytes");
                if (this.n.size == 0) {
                    ...
                    this.l.releaseOutputBuffer(i5, false);
                    if (this.r) {
                        e.c(false, b, "write " + this.m.write(this.o, 0, i4) + " shorts to the audio sink. Taken Time=" + (System.currentTimeMillis() - System.currentTimeMillis()));
                    }
    ...
    public void onAudioFocusChange(int i) {
        switch (i) {
            case -3:
                e.b(b, "audio focus: loss transient can duck");
                a(0.10000000149011612d, 0.10000000149011612d);
                return;
            case -2:
                e.b(b, "audio focus: loss transient");
                return;
            case -1:
                e.b(b, "audio focus: loss");
                return;
            case 1:
                e.b(b, "audio focus: gain");
                this.r = true;
                a(1.0d, 1.0d);
                return;
            default:
