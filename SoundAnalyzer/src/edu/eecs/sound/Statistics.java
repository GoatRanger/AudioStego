...
public class Statistics {
	 ...
	 * Added by Karl Gossett (not part of the original Statistics.java
	 * @param xData
	 * @param yData
	 * @return
	 */
	public static double getMaximum(Number[] data) {
		double max = 0.0;
		for (int counter = 0; counter < data.length; counter++) {
			if (data[counter].doubleValue() > max) {
				max=data[counter].doubleValue();
			}
		}
		return max;
	}
	 ...
	 * @return
	 */
	public static double getMinimum(Number[] data) {
		double min = Double.MAX_VALUE;
		for (int counter = 0; counter < data.length; counter++) {
			if (data[counter].doubleValue() < min) {
				min=data[counter].doubleValue();
			}
		}
		return min;
	}
