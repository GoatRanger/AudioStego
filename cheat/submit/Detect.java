...
class Detect {
  ...
  public static void main (String[] args) throws IOException {
	...
	if (f1.exists()&&f2.exists()) {
	  ...
	  while (bf1.ready())
	    sequence1.addElement(bf1.readLine());

	  sequence2.addElement("");
	  while (bf2.ready()) 
	    sequence2.addElement(bf2.readLine());
  ...
  private static int getSubstVal(String s1, String s2) {
    ...
    if (s1.equals(s2))
      return 5;
    else if (containsKeyword(s1) && containsKeyword(s2))
      return -10;
    else if (containsKeyword(s1) && !containsKeyword(s2)) 
      return -10;
    else if (!containsKeyword(s1) && containsKeyword(s2)) 
      return -10;
    else if (containsSymbol(s1) && containsSymbol(s2)) 
      return -10;
    else if (containsSymbol(s1) && !containsSymbol(s2)) 
      return -10;
    else if (!containsSymbol(s1) && containsSymbol(s2)) 
  ...
  private static boolean containsKeyword(String s) {
  ...
  final static String[] keywords = { "abstract", "boolean", "break", "byte", 
