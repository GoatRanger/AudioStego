...
class InterferenceFrame extends Frame implements ComponentListener, ActionListener, ItemListener, AdjustmentListener,
	...
	public void init() {
		...
		add(brightnessBar = new Scrollbar(Scrollbar.HORIZONTAL, 280, 1, 1, 1000));
	...
	void doPlay() {
		...
		// System.out.print(b[0] + " " + b[2] + " " + b[blockAdder-2] + " " +
		// b[blockAdder] + " " + b[blockAdder+2] + "\n");
	...
	void centerString(Graphics g, String s, int y) {
		...
		g.fillRect((winSize.width - 8 - w) / 2, y - fm.getAscent(), w + 8, fm.getAscent() + fm.getDescent());
	...
	public void updateInterference(Graphics realg) {
		...
		for (x = 0; x != gridSize; x++)
			for (y = 0; y != gridSize; y++) {
				...
				r1 *= k;
				r2 *= k;
				double q1 = r1s * java.lang.Math.sin(r1) + r2s * java.lang.Math.sin(r2 + phase);
				double q2 = r1s * java.lang.Math.cos(r1) + r2s * java.lang.Math.cos(r2 + phase);
		...
		centerString(g, "Wavelength = " + convertUnits(34500 / getFreq()), winSize.height - 80);
		centerString(g, "Speaker separation = " + convertUnits(speakerSep), winSize.height - 60);
		centerString(g, "Phase difference = " + (int) (phase * 180 / pi) + "\u00b0", winSize.height - 40);
		if (dragging) {
	...
	public void adjustmentValueChanged(AdjustmentEvent e) {
		...
		if (e.getSource() == freqBar || e.getSource() == phaseBar || e.getSource() == balanceBar)
			doPlay();
		cv.repaint();
	}
	...
	public void itemStateChanged(ItemEvent e) {
		if (e.getItemSelectable() == soundCheck || e.getItemSelectable() == stereoCheck) {
			doPlay();
			if (!stereoCheck.getState()) {
				balanceBar.disable();
				phaseBar.disable();
	...
	public void run() {
		try {
			...
			while (true) {
				...
				int i;
				int len = (bufferSize + offset > blockAdder) ? blockAdder - offset : bufferSize;
				for (i = 0; i != len; i++)
